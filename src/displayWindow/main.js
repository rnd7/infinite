...
const TEX_PATH = "../../assets/"


let textures = []

const loadTextures = function(p) {
  fs.readdir(
...
function makeInfiniteZoom() {
  ...
  t.camera.position.z = 0;
  t.camera.updateProjectionMatrix()


  t.layer = new THREE.Group()
  ...
  t.createSurface = function(startPosition) {
        ...
        new THREE.MeshBasicMaterial(
          {
            ...
            color: Math.floor(t.minColor + Math.random() * (0xFFFFFF-t.minColor)),
            side: THREE.DoubleSide,
            transparent: true
          }
        )
      )
      mesh.position.z = startPosition
      mesh.rotation.z = Math.random() * 2 * Math.PI
      mesh.userData.rotationSpeed = (Math.random() * 2 - 1) * t.angleSpeed
      return mesh
  }

  t.remove = function(object) {
    //console.log("remove")
    t.layer.remove(object)
    object.material.dispose()
  }

  t.removeAll = function() {
    for(var i=0; i<t.layer.children.length; i++) {
      t.layer.remove(t.layer.children[i])
    }
  }
  ...
  t.animate = function() {
    for(var i=0; i<t.layer.children.length; i++) {
      const object = t.layer.children[i]
      if(object.position.z > t.killPosition ) {
        t.remove(object)
      } else {
        object.position.z += t.speed
        object.rotation.z += object.userData.rotationSpeed
      }
    }
    if (!t.layer.children.length || t.layer.children[t.layer.children.length-1].position.z > t.spawnPoint+t.spawnDistance) {
      if (textures.length) t.layer.add(t.createSurface(t.spawnPoint))
    }
  }
